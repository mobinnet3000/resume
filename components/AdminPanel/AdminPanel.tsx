...
import { FiX, FiLock, FiSave, FiPlus, FiTrash2, FiDownload, FiUpload } from 'react-icons/fi'
import { checkAuth, useAdmin, getConfigCode, publishConfig } from '@/lib/admin-store'
          ...
          >
            ...
            {!authed ? (
              ...
              <div className="space-y-5">
                ...
                <div className="flex flex-col gap-3 pt-4 border-t" style={{ borderColor: 'var(--surface-border)' }}>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <button
                      onClick={async () => {
                        const ok = await publishConfig(data)
                        if (ok) alert('Published for everyone!')
                        else alert('Failed to publish. Changes saved locally.')
                      }}
                      className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-medium transition-all"
                      style={{ backgroundColor: 'var(--syntax-string)', color: '#000' }}
                    >
                      <FiUpload className="w-3.5 h-3.5" /> Publish for Everyone
                    </button>
                    <button
                      onClick={() => {
